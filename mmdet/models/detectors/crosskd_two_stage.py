...
class CrossKDTwoStageDetector(TwoStageDetector):
    ...
    def __init__(self,
        ...
        self.reused_teacher_head_idx = kd_cfg['reused_teacher_head_idx'] # lấy lớp trung gian thứ 
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # Căn chỉnh `i` sao cho giống với `i+1`
        aligned_stu_x_i_resized = F.interpolate(stu_x[1], size=stu_x[self.reused_teacher_head_idx].shape[2:], mode='bilinear', align_corners=False)

        T = aligned_stu_x_i_resized + stu_x[self.reused_teacher_head_idx]
        # Channel Attention
        b, c, h, w = T.size()
        gap = F.adaptive_avg_pool2d(T, 1).view(b, c)                          # (B, C)
        fc1 = F.relu(F.linear(gap, torch.randn(c // 16, c).to(gap.device)))  # FC giảm chiều
        fc2 = torch.sigmoid(F.linear(fc1, torch.randn(c, c // 16).to(gap.device)))  # FC tăng chiều


        channel_weight = fc2.view(b, c, 1, 1)                                 # (B, C, 1, 1)
        T = T * channel_weight                                               # Apply Channel Attention


        # Spatial Attention
        avg_out = torch.mean(T, dim=1, keepdim=True)                          # (B, 1, H, W)
        max_out, _ = torch.max(T, dim=1, keepdim=True)                        # (B, 1, H, W)
        spatial = torch.cat([avg_out, max_out], dim=1)                        # (B, 2, H, W)

        # Convolution kernel (random nếu không dùng lớp học)
        conv_weight = torch.randn(1, 2, 7, 7).to(T.device)                    # kernel 7x7
        spatial_att = F.conv2d(spatial, conv_weight, padding=3)              # (B, 1, H, W)
        spatial_att = torch.sigmoid(spatial_att)

        T = T * spatial_att                                                  # Apply Spatial Attention
        ...
        if self.with_feat_distill:
            losses_feat_kd = [
                ...
                for feat, tea_feat in zip(T, tea_x)
            ]
